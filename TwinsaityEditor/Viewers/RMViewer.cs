        ...
        public void LoadInstances()
        {
            ...
            for (uint i = 0; i <= 7; ++i)
            {
                ...
                if (((TwinsSection)file.Data.GetItem(i)).RecordIDs.ContainsKey(6))
                {
                    foreach (Instance ins in ((TwinsSection)((TwinsSection)file.Data.GetItem(i)).GetItem(6)).Records)
                    {
                        Matrix3 rot_ins = Matrix3.CreateRotationZ(-ins.RotZ / (float)(ushort.MaxValue + 1) * 2f * MathHelper.Pi);
                        rot_ins *= Matrix3.CreateRotationY(-ins.RotY / (float)(ushort.MaxValue + 1) * 2f * MathHelper.Pi);
                        rot_ins *= Matrix3.CreateRotationX(ins.RotX / (float)(ushort.MaxValue + 1) * 2f * MathHelper.Pi);
